...
| QA-01 | Юнит-тесты pytest, покрытие ≥50% | Готово | CI запускает pytest с порогом покрытия 50%; добавлены тесты upload/search/parser/es_client, включая проверку парсинга фикстур. |
...
| QA-04 | Нагрузочные тесты на 50 пользователей | Готово | Добавлены сценарий и отчёт; 50/50 поисковых запросов выполнены успешно. |
| QA-05 | Precision@3 для 10 запросов | Готово | Добавлен оценочный сценарий и отчёт; Precision@3 = 1.00 на 10 эталонных запросах. |
| QA-06 | Руководство пользователя | Готово | Руководство обновлено под доступные API загрузки, поиска и списка документов; ручной API-flow проверен через Docker Compose. |
