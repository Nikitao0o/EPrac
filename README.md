...
|----|------------|--------|-------------|
| BE-01 | `POST /api/v1/documents/upload` | Готово | Реализован endpoint загрузки одного файла. |
...
| BE-08 | `GET /api/v1/search?q={query}` multi-match | Не начато | Search endpoint пока не добавлен. |
...
| API-02 | Статусы: 200, 400, 404, 500 | Частично | 200/400 есть в upload; остальные нужны в следующих endpoint. |
...
| DO-06 | Prometheus + Grafana | Частично | Сервисы и `/metrics` добавлены; отдельные метрики `/search` появятся после search endpoint. |
| DO-07 | `init.sh` для скачивания и загрузки 10 PDF-лекций | Не начато | Скрипт пока не добавлен. |
| QA-01 | Юнит-тесты pytest, покрытие ≥50% | Не начато | Тестов пока нет. |
| QA-02 | E2E Playwright: загрузка → индексация → поиск → вывод | Не начато | Зависит от frontend и полного backend flow. |
| QA-03 | Набор тестовых документов | Не начато | Fixtures пока не добавлены. |
...
| QA-06 | Руководство пользователя | Не начато | User Guide пока не добавлен. |
